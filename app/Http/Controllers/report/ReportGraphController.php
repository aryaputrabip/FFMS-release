...
class ReportGraphController extends Controller
{
    ...
    public function index(Request $r){
        ...
        if(isset($role)){
            ...
            return view('report.index', compact('title','username','role','app_layout','filter_year_available'));
        }
    }
        ...
        switch($r->FILTER_TYPE){
            case "daily":
                ...
                for($i=1; $i <= $total_loop; $i++){
                    if($r->FILTER_MONTH == "all"){
                        if($r->FILTER_YEAR == "all"){
                            $result_daily = CutiMemberModel::whereDay('created_at', $i)->count();
                        }else{
                            $result_daily = CutiMemberModel::whereDay('created_at', $i)->whereYear('created_at', (int)$r->FILTER_YEAR)->count();
                        }
                    }else{
                        if($r->FILTER_YEAR == "all"){
                            $result_daily = CutiMemberModel::whereDay('created_at', $i)->whereMonth('created_at', (int)$r->FILTER_MONTH)->count();
                        }else{
                            $result_daily = CutiMemberModel::whereDay('created_at', $i)->whereMonth('created_at', (int)$r->FILTER_MONTH)->whereYear('created_at', (int)$r->FILTER_YEAR)->count();
                        }
                    }
            ...
            case "monthly":
                ...
                for($i=1; $i <= $total_loop; $i++){
                    ...
                    if($r->FILTER_YEAR == "all"){
                        $result_monthly = CutiMemberModel::whereMonth('created_at', $i)->count();
                    }else{
                        $result_monthly = CutiMemberModel::whereMonth('created_at', $i)->whereYear('created_at', (int)$r->FILTER_YEAR)->count();
                    }
            ...
            case "yearly":
                ...
                for($i=$year_start; $i < $year_start + $total_loop; $i++){
                    $result_yearly = CutiMemberModel::whereYear('created_at', $i)->count();
            ...
            case "daily":
                ...
                for($i=1; $i <= $total_loop; $i++){
                    if($r->FILTER_MONTH == "all"){
                        ...
                        if($r->FILTER_YEAR == "all"){
                            $result_daily = MemberLogModel::whereDay('created_at', $i)->whereMonth('date', (int)$r->FILTER_MONTH)->sum('transaction');
                            ...
                            $result_daily = MemberLogModel::whereDay('created_at', $i)->whereMonth('date', (int)$r->FILTER_MONTH)->whereYear('date', (int)$r->FILTER_YEAR)->sum('transaction');
